...
class DoltDeclarationTests(unittest.TestCase):
    ...
    def test_unhashable_evidence_version_fails_with_declaration_error(self) -> None:
        baseline = json.loads(
            (ROOT / "sql/dialect-profile-baseline.json").read_text()
        )
        declaration = {
        ...
        with self.assertRaisesRegex(
            DoltDeclarationError, "unique exact versions"
        ):
            validate_dolt_declaration(
                declaration,
                authoritative_profile=baseline["profiles"]["dolt"],
    ...
    def test_declaration_and_run_ids_are_independently_unique(self) -> None:
        for duplicated_field in ("declaration_id", "conformance_run_id"):
            with self.subTest(duplicated_field=duplicated_field):
                with tempfile.TemporaryDirectory() as temporary:
                    root = Path(temporary)
                    for relative in (
                        "sql/dialect-profile-baseline.json",
                    ...
                    declaration = {
                        ...
                        "profile": copy.deepcopy(
                            baseline["profiles"]["dolt"]
                        ),
                        "specification_commit": "a" * 40,
                    }
                    second = copy.deepcopy(declaration)
                    second["declaration_id"] = "python-dolt-2.2.2-second"
                    second["conformance_run_id"] = "run-20260803-second"
                    second[duplicated_field] = declaration[duplicated_field]
                    declarations = (
                        root / "sql/dolt-adapter-declarations"
                    )
                    (declarations / "one.json").write_text(
                        json.dumps(declaration)
                    )
                    (declarations / "two.json").write_text(json.dumps(second))
